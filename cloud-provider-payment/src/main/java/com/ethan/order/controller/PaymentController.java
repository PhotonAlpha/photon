...
public class PaymentController {
	...
	public ResponseEntity<CommonResult> createPayment(@PathVariable("serial") String serial) {
		...
		CommonPayment commonPayment = paymentService.createPayment(serial);
		CommonResult success = new CommonResult(200, "插入数据成功，serverPort：" + serverPort, commonPayment);
		return ResponseEntity.ok(success);
	}
	@GetMapping("/payment")
	public ResponseEntity<CommonResult> getPayment() {
		log.info("getPayment**********");
		CommonResult success = new CommonResult(200, "插入数据成功，serverPort：" + serverPort, UUID.randomUUID().toString());
		return ResponseEntity.ok(success);
	}
